...
Laravel MCP Server — крутой пакет для лёгкого создания MCP серверов в приложениях на Laravel. **В отличие от других MCP пакетов для Laravel, которые используют stdio**, этот пакет делает упор на **Streamable HTTP** и сохраняет **устаревший SSE-провайдер** для совместимости, обеспечивая безопасную и контролируемую интеграцию.
...
- **Контроль**: Streamable HTTP даёт лучший контроль над общением между LLM клиентами и вашим приложением

Используя Streamable HTTP для MCP сервера, вы можете:
...
- Быстрое внедрение Streamable HTTP в существующие проекты на Laravel
...
- Поддержка коммуникации в реальном времени через Streamable HTTP и SSE (устаревший провайдер)
...
Опцию `server_provider` в конфигурации можно установить в один из двух режимов:

1. **streamable_http** – рекомендованный по умолчанию. Использует обычные HTTP-запросы и подходит для серверless-платформ, где SSE-соединения обрываются примерно через минуту.
2. **sse** – устаревший провайдер для обратной совместимости. Требует длительных SSE-соединений и может не работать на таких платформах.

Режим «Streamable HTTP SSE», описанный в протоколе MCP, в пакете не реализован и пока не планируется.
...
1. **ВНИМАНИЕ**: НЕВОЗМОЖНО использовать `php artisan serve` с этим пакетом, поскольку он не может обрабатывать несколько PHP-соединений одновременно. Поскольку MCP SSE требует обработки нескольких соединений одновременно, Вы должны использовать одну из этих альтернатив:
   ...
   * **Laravel Octane** (самый простой вариант):
      ...
      # Запуск сервера Octane
      php artisan octane:start
      ```
      
      > **Важно**: При установке Laravel Octane убедитесь, что вы используете FrankenPHP в качестве сервера. Пакет может не работать правильно с RoadRunner из-за проблем совместимости с соединениями SSE. Если вы можете помочь решить эту проблему совместимости с RoadRunner, пожалуйста, отправьте Pull Request - ваш вклад будет очень ценен!
   ...
   * **Продакшн-варианты**:
     - Nginx + PHP-FPM
     - Apache + PHP-FPM
     - Собственная настройка Docker
    - Любой веб-сервер с правильной поддержкой стриминга SSE (нужно только при использовании устаревшего SSE-провайдера)

2. В интерфейсе Инспектора введите URL конечной точки MCP вашего сервера Laravel (например, `http://localhost:8000/mcp`). Если используется устаревший SSE-провайдер, укажите URL SSE (`http://localhost:8000/mcp/sse`).
3. Подключитесь и исследуйте доступные инструменты визуально

Конечная точка MCP выглядит так: `http://[ваш-laravel-сервер]/[default_path]`, где `default_path` указан в файле `config/mcp-server.php`.

## Расширенные возможности

### Архитектура Pub/Sub с адаптерами SSE (устаревший провайдер)
...
3. **Подписчик (Соединение SSE)**: Долговременные SSE-соединения подписываются на сообщения для соответствующих клиентов и доставляют их в реальном времени. Это применимо только при использовании устаревшего SSE-провайдера.
