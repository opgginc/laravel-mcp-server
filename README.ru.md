...
<p align="center">
  Мощный Laravel пакет для бесшовного создания сервера Model Context Protocol
</p>
...
Версия 1.1.0 внесла значительные и критические изменения в `ToolInterface`. Если вы обновляетесь с v1.0.x, вы **обязательно** должны обновить свои реализации инструментов в соответствии с новым интерфейсом.
...
2.  **Создать резервные копии:** Перед внесением изменений она создаст резервную копию вашего оригинального файла инструмента с расширением `.backup` (например, `YourTool.php.backup`). Если файл резервной копии уже существует, оригинальный файл будет пропущен, чтобы предотвратить случайную потерю данных.
...
После обновления пакета `opgginc/laravel-mcp-server` до v1.1.0 или новее, если у вас есть существующие инструменты, написанные для v1.0.x, настоятельно рекомендуется запустить эту команду:
...
Если вы предпочитаете мигрировать свои инструменты вручную, вот сравнение, которое поможет вам адаптировать существующие инструменты:
...
- **Защита данных**: Организациям необходимо защищать проприетарные API endpoints и внутреннюю архитектуру системы
...
- Раскрывать только необходимые инструменты и ресурсы, сохраняя конфиденциальность проприетарных деталей API
...
- Поддержка коммуникации в реальном времени через Streamable HTTP с интеграцией SSE
...
- Простая конфигурация маршрутизации и middleware
...
MCP протокол также определяет режим "Streamable HTTP SSE", но этот пакет его не реализует и не планирует этого делать.
...
**Когда использовать ограничение по доменам:**
...
interface ToolInterface
{
    ...
    // Определяет ожидаемые входные параметры для вашего инструмента, используя структуру, похожую на JSON Schema.
...
- **`readOnlyHint`** (boolean): Указывает, только ли инструмент читает данные без изменения окружения (по умолчанию: false)
...
**Важно:** Это подсказки, а не гарантии. Они помогают клиентам обеспечивать лучший пользовательский опыт, но не должны использоваться для критически важных для безопасности решений.
...
public function annotations(): array
{
    return [
        ...
        'idempotentHint' => false,     // Двойное удаление имеет разные эффекты
        'openWorldHint' => false,
    ];
}
...
- Выполняя инструмент с предоставленным вами входом
...
2. **Message Broker (Адаптер)**: Адаптер (например, Redis) поддерживает очереди сообщений для каждого клиента, идентифицируемые уникальными ID клиентов. Это обеспечивает надежный асинхронный коммуникационный слой.

3. **Subscriber (SSE соединение)**: Долгоживущие SSE соединения подписываются на сообщения для соответствующих клиентов и доставляют их в реальном времени. Это применимо только при использовании legacy SSE провайдера.
