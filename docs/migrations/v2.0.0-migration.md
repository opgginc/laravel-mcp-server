...
4. **Legacy tool transport methods were removed**
   ...
   - Remove `isStreaming(): bool` (if present in your tools)
...
3. Move middleware settings from config to route groups.
4. Move tool/resource/template/prompt registrations to the route builder chain.
5. Remove legacy `messageType()`/`isStreaming()` methods from tools (or run the migration command).
...
| `server.*` (combined) | `->setServerInfo(name: ..., version: ..., title: ..., description: ..., websiteUrl: ..., icons: ..., instructions: ...)` |
| `tools` | `->tools([...])` |
| `resources` | `->resources([...])` |
| `resource_templates` | `->resourceTemplates([...])` |
| `prompts` | `->prompts([...])` |
| `tool_capabilities.list_changed` | `->toolListChanged(true)` |
| `resources capability flags` | `->resourcesSubscribe(true)->resourcesListChanged(true)` |
| `prompts capability flags` | `->promptsListChanged(true)` |
...
This command supports v1.0.x/v1.1.x/v1.2.x tool signatures and removes obsolete `messageType()` usage while preserving backups by default.

### Manual example

```php
// Remove legacy method/imports from your tool class
